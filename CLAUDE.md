...
- **Concurrent backfill prevention**: MySQL advisory locks `gallerykit_color_pipeline_backfill` and `gallerykit_semantic_embedding_backfill` are acquired on dedicated connections for their respective color-pipeline and semantic-embedding backfill windows. Concurrent invocations serialize cleanly rather than racing the same image rows.
