...
| `apps/web/src/lib/process-image.ts` | Sharp pipeline (parallel AVIF/WebP/JPEG, ICC parsing, bounds checks). `IMAGE_PIPELINE_VERSION` (currently 8) is DEFINED in `gallery-config-shared.ts` (search for `export const IMAGE_PIPELINE_VERSION`) and re-exported here |
...
| `pipeline_version` | Encoder version used to produce derivatives (current: 8) | admin-only |
| `derivative_max_width` | Actual maximum pixel width delivered by the current derivative set | public-safe — drives truthful responsive `w` descriptors; null legacy WI-15 rows use the base JPEG until re-encoded |
