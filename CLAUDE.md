...
- `admin_tokens` - Lightroom Classic publish-plugin PATs (US-P53). Each token is a 32-char random hex string scoped to one admin user, stored hashed (SHA-256) in the DB. The plugin (`/api/admin/lr/upload`) accepts the token in an `X-Admin-Token` header and creates images directly without a session cookie. Tokens can be rotated or revoked from the admin Settings panel. The LR upload route has a dedicated nginx body-size location (216 MiB) that wins over the generic `/api/admin/` 2 MiB catch-all by longest-prefix match.
- `smart_collections` - Admin-defined dynamic galleries (US-P42). Each row stores a name, slug, and a JSON `rules` array that defines matching criteria (e.g., topic, tag, date range, color pipeline decision). Photos are matched dynamically at query time; no materialized join table. The public route `/s/[slug]` renders a smart collection the same way as a topic gallery. Smart collection mutations (create, update, delete) are gated by `getRestoreMaintenanceMessage()` like all other mutating admin actions.
...
- **OG route SSRF hardening:** both OG image routes (`api/og/route.tsx` and `api/og/photo/[id]/route.tsx`) construct the `new URL()` for the card's background image using a fail-closed pattern: if `siteConfig.siteUrl` is missing or invalid, the route returns a 404 rather than falling back to a relative URL that could be manipulated into an unintended fetch target. The `siteUrl` is validated at startup (must be absolute HTTPS without credentials), so a misconfigured deployment fails loud at build time rather than silently serving broken OG cards. The per-photo route additionally validates the photo ID exists before rendering, so a non-existent ID returns 404 instead of a card with a broken image reference.
...
6. Per-format **fresh** `sharp(inputPath, …)` instance (WI-14 cross-format isolation — see the Color & HDR "Encoder decision matrix" note), with `clone()` used only WITHIN a format (e.g. the 10-bit AVIF fallback). NOTE (AGG-R7-08): the encoder does NOT keep a single decoded instance across formats/sizes — it opens a fresh decode per output to eliminate shared-state contamination, trading decode reuse for correctness (`process-image.ts:1131-1135`)
...
3. Copy `apps/web/src/site-config.example.json` to `apps/web/src/site-config.json` and customize it; deploy/build paths now fail fast if the real file is missing. The file is a flat JSON object with these key fields:
   - `siteName` — displayed in nav, footer, and OG title
   - `siteDescription` — OG description fallback
   - `siteUrl` — canonical base URL (must match `BASE_URL` env var)
   - `authorName` / `authorUrl` — Atom feed attribution
   - `social.*` — optional social link URLs (GitHub, Instagram, etc.)
   - `navLinks` — array of `{ label, href }` objects for the top nav
   - `footerLinks` — array of `{ label, href }` objects for the footer
