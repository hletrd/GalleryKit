...
- Admin DB backup/restore is SQL-only. It snapshots and restores database rows, then runs the committed migration/reconcile postconditions after import, but it does not snapshot or roll back host files in `data/originals`, `public/uploads`, or `public/resources`. Use host-level filesystem backups/reconciliation for full rollback.
