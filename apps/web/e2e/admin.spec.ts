...
import { ensureEnglishLocale, expectNoNextError, loginAsAdmin } from './helpers';
...
test('admin upload workflow works on the dashboard', async ({ page }) => {
  ...
  const uploadPath = path.resolve(process.cwd(), 'apps/web/public/uploads/original/e2e-landscape.jpg');
  const jpegBuffer = await fs.readFile(uploadPath);

  await page.locator('#admin-content select').selectOption('e2e-smoke');
