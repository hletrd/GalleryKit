...
- File-backed `src/site-config.json` owns static links, fallback metadata, and analytics defaults. Next.js inlines these JSON imports at build time, so changing `home_link`, `footer_text`, `url`, or `google_analytics_id` requires a rebuild/deploy. Admin-editable SEO and branding fields are stored in the database and override file defaults at runtime; `seo_locale` is an OpenGraph locale fallback, while normal HTML `lang` comes from the route locale. Leave `google_analytics_id` empty to keep analytics fully first-party/self-hosted; setting it loads Google Analytics on public pages and is a deliberate third-party analytics opt-in.
