                    ...
                    {initialTags.map((tag) => (
                        <TableRow key={tag.id}>
                            ...
                            <TableCell className="text-right space-x-2">
                                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditError(null); setEditingTag(tag); }} aria-label={t('tags.editTagAria', { name: tag.name })}>
                                    <Pencil className="h-4 w-4" />
                                </Button>
                                <Button variant="ghost" size="icon" className="text-destructive-text h-8 w-8" onClick={() => setDeleteId(tag.id)} aria-label={t('tags.deleteTagAria', { name: tag.name })}>
