...
export default function PhotoViewer({ images, initialImageId, prevId, nextId, canShare = false, isAdmin = false, isSharedView = false, syncPhotoQueryBasePath, imageSizes = DEFAULT_IMAGE_SIZES, siteTitle = siteConfig.title, shareBaseUrl = siteConfig.url, untitledFallbackTitle, showDocumentHeading = true, slideshowIntervalSeconds = 5, forceShowColorChips = false, forceSrgbDerivatives = false, semanticSearchMode = 'disabled' }: PhotoViewerProps) {
    ...
    const primaryPhotoAccessibleName = getConcisePhotoAltText(image, t('common.photo'));
            ...
            <div className={cn(
                ...
                <div ref={mediaContainerRef} className="relative flex items-center justify-center bg-black/5 dark:bg-black rounded-xl border dark:border-transparent p-2 overflow-hidden min-h-[40vh] md:min-h-[500px] group">
                    <PhotoNavigation
                        ...
                        swipeTargetRef={mediaContainerRef}
                    />
                    ...
                    <AnimatePresence mode="wait" initial={false}>
                        ...
                        >
                            <div className="w-full h-full flex items-center justify-center">
                                <ImageZoom className="w-full h-full flex items-center justify-center" accessibleName={primaryPhotoAccessibleName}>
                                    {srcSetData}
                                </ImageZoom>
                            </div>
                        </motion.div>
                                ...
                                <dl className="grid grid-cols-2 gap-y-4 gap-x-2 text-sm">
                                    ...
                                    {hasExifData(image.exposure_time) && (
                                        <div>
                                            ...
                                            <dd className="font-medium">{formatShutterSpeed(image.exposure_time)}</dd>
                                        </div>
                                    )}
