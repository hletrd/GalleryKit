...
const {
    ...
    was_downscaled: _omitWasDownscaled,
...
const {
    ...
    was_downscaled: _omitWasDownscaledMap,
...
// (`api/search/semantic/route.ts`, `api/search/similar/[id]/route.ts`). Both
// routes previously hand-copied an identical inline `db.select({...})` for
// result-card enrichment, bypassing the `Extract<…, PrivacySensitiveKeys>`
// compile guard that `publicSelectFields` / `searchFields` carry. That made
// these the ONLY public image-select surfaces protected at *test* time
...
// deletes the duplication. `topic_label` is a JOINed column (topics.label), not
// an images column — it is not privacy-sensitive and not in PrivacySensitiveKeys.
...
type _SearchEnrichmentSensitive = Extract<keyof typeof searchEnrichmentSelectFields, _PrivacySensitiveKeys>;
