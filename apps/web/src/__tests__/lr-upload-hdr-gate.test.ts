...
describe('lr upload parity source-contract (cycle 4)', () => {
    ...
    it('bounds concurrent multipart body parsing before request.formData()', () => {
        ...
        const formDataIndex = LR_SRC.indexOf('request.formData()');
        const countQuotaIndex = LR_SRC.indexOf('tracker.count + 1 > UPLOAD_MAX_FILES_PER_WINDOW');
        const byteQuotaIndex = LR_SRC.indexOf('tracker.bytes + declaredUploadBytes > MAX_TOTAL_UPLOAD_BYTES');
        const trackerPreclaimIndex = LR_SRC.indexOf('tracker.count += 1');
        expect(acquireIndex).toBeGreaterThan(-1);
        expect(formDataIndex).toBeGreaterThan(-1);
        expect(countQuotaIndex).toBeGreaterThan(-1);
        expect(byteQuotaIndex).toBeGreaterThan(-1);
        expect(trackerPreclaimIndex).toBeGreaterThan(-1);
        expect(countQuotaIndex).toBeLessThan(acquireIndex);
        expect(byteQuotaIndex).toBeLessThan(acquireIndex);
        expect(acquireIndex).toBeLessThan(formDataIndex);
        expect(acquireIndex).toBeLessThan(trackerPreclaimIndex);
        expect(LR_SRC.slice(trackerPreclaimIndex, formDataIndex)).not.toMatch(/return\s+NextResponse/);
