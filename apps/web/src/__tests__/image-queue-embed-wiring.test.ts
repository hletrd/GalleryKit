...
describe('upload embedding hook wiring', () => {
  ...
  it('resolves current semanticSearchMode before post-processing embedding writes', () => {
    ...
    expect(src).toMatch(
      /const cfg = await getGalleryConfigDetached\(\);[\s\S]*semanticMode = applyRuntimeSemanticGate\(cfg\.semanticSearchMode\);[\s\S]*storeImageEmbeddingForMode\(job\.id, originalPath, semanticMode\)/,
    );
