...
| TEST-R4C9-04 | MED(gap)/High | `timelineSelectFields` privacy mirror has NO compile-time guard and NO fixture pin (unlike data.ts) — a future PII column addition to the public timeline/OnThisDay/year-review surface compiles + passes tests silently. Fix: add the data.ts-style type guard + extend the privacy fixture suite | test, security |
